    ...
    template: `
        <details class="stats-panel">
            <summary>
                ...
                <span class="count">{{ totalTrackeadas }} series</span>
            </summary>
            <div class="stats-panel-content">
                    ...
                    <div v-if="mejorValoradas.length">
                        ...
                        <ol class="stats-rank-list">
                            <li v-for="s in mejorValoradas" :key="s.titulo">
                                {{ s.titulo }} <span class="nota-tag" :class="s.notaClase">{{ s.nota }}</span>
                            </li>
                        </ol>
                    </div>
                    <div v-if="peorValoradas.length">
                ...
                <div class="stats-section stats-section--split" v-if="masRevisitadas.length">
                    <div>
                        <h3 class="stats-section-title">Más revisitadas</h3>
                        <ol class="stats-rank-list">
                            <li v-for="s in masRevisitadas" :key="s.titulo">
                                {{ s.titulo }} <b>x{{ s.veces }}</b>
                            </li>
                        </ol>
                    </div>
                    <div>
                        <h3 class="stats-section-title">Rewatch vs. primera vez</h3>
                        <p class="stats-inline-stat">Nota media rewatch: <span class="nota-tag" :class="getNotaClass(notaMediaRewatch)">{{ notaMediaRewatch ?? '—' }}</span></p>
                        <p class="stats-inline-stat">Nota media 1ª vez: <span class="nota-tag" :class="getNotaClass(notaMediaPrimeraVez)">{{ notaMediaPrimeraVez ?? '—' }}</span></p>
                    </div>
                </div>
